...
struct RestaurantPhotosView: View {
    ...
    @State var mode = "grid"
    init() {
        // this changes every UISegmentedControl
...
//                                /*@START_MENU_TOKEN@*/Text("Button")/*@END_MENU_TOKEN@*/
                                KFImage(URL(string: urlString))
                                    .resizable().scaledToFill().frame(width: proxy.size.width / 3 - 3, height: proxy.size.width / 3 - 3).clipped()
                            })
                        }
                    ...
                    ForEach(photoUrlStrings, id:\.self) { urlString in
                        VStack(alignment: .leading, spacing: 8) {
                            ...
                            .font(.system(size:22))
                            Text("Description for your post make suer you use a bunch of lines of text otherwise you won't know if it wraps apporopriately or if it's gonna just stick.\n\nGreate job")
                                .font(.system(size:14))
                                .padding(.horizontal, 8)
                            Text("Posted on December 10th, 2020")
                                .font(.system(size:14))
                                .padding(.horizontal, 8)
